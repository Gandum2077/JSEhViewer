...
启动失败时的只读恢复页、恢复说明和脱敏诊断导出已经建立，并通过 Node fixture、production bundle 构建和 2026-08-12 JSBox 真机隔离诊断（16 ms）。恢复页不会自动用备份覆盖当前数据库，诊断也不读取业务行。下一小步是审计现有业务入口与五个 v2 adapter 的签名兼容性。虽然临时迁移和五个 adapter 都已通过真机隔离验证，也不能立刻更改 `CURRENT_USER_VERSION`；否则仍有未切换的 App 路径可能查询已经不存在的 v1 表。
