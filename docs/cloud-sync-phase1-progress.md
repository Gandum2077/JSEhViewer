...
1. 在隔离临时库中装配五个 v2 正式 Repository，并补齐 ConfigManager 关键调用路径的整库回归；
2. 整库回归通过后，再设计正式 Repository 工厂、迁移后 seed 顺序和启动切换；`CURRENT_USER_VERSION` 在此之前保持 1。
