...
> 开始日期：2026-08-11
> 当前状态：进行中，已接近本阶段收尾。SQLite 安全层、数据库初始化、DB v2 schema 与 v1 → v2 迁移均已通过自动验证和 JSBox 真机临时库验证；图库、搜索历史与书签、标记与屏蔽上传者、marked tags Repository 兼容层以及共享 HLC / `sync_versions` / `sync_outbox` 原子写入内核也已通过真机验证。图库、标记上传者、搜索历史、搜索书签和本地标签五个实际 v2 adapter，以及数据库启动失败恢复与脱敏诊断，均已通过自动与真机隔离验证。v1/v2 正式业务契约、搜索稳定 ID facade 和业务文件直接 SQL 扫描，以及五类 v2 Repository 的集中装配和关键业务路径整库回归，也已通过自动与 JSBox 真机隔离验证。备份、迁移、runtime、可重入 seed 与就绪复核已集中为启动状态机，并通过自动故障注入与 JSBox 真机隔离验证。正式数据库尚未迁移，也未上传业务数据。
...
- 2026-08-12，该隔离诊断已在 JSBox 真机通过（27 ms）：稳定字符串 ID 完整穿过 Config/UI facade，历史与书签写入、本机删除、最近搜索词、重排、tombstone、旧数字 ID 拒绝、关闭重开和临时文件清理均符合预期。

## 已完成：第十八步（五类 v2 Repository 集中装配与整库回归）

- 新增 `RepositoryRuntime` 集中装配层。正式 `src/repositories/index.ts` 也通过该层导出四个业务 Repository，但目前仍明确调用 v1 工厂；未来不允许在多个页面分散判断数据库版本。
- `createV2RepositoryRuntime()` 在同一数据库、HLC writer 和 codec 下装配图库、搜索历史、搜索书签、上传者与本地标签五个 Adapter，并把搜索重新组合成 ConfigManager 所需的单一接口。
- 新增跨域整库回归：7 个迁移对象可重入 seed，并按 ConfigManager/StatusManager 的实际公共 Repository 调用顺序覆盖图库、搜索、上传者与标签双模式。
- 整库回归确认本机下载不产生 outbox、搜索历史本机删除不产生 tombstone、上游屏蔽上传者隔离冲突标记、重新登录整表清空标签和同步状态。
- 所有剩余 outbox 必须与 `sync_versions` 的 object key、HLC、device、deleted、op ID 一致；五类 Adapter 的 HLC 不得重复。
- 新增 `npm run test:repository-runtime-v2` 与真机隔离入口“检查五类 v2 Repository 整库装配”。详细边界见 [`cloud-sync-repository-runtime-v2.md`](cloud-sync-repository-runtime-v2.md)。
...
- 2026-08-12，该隔离诊断已在 JSBox 真机通过（418 ms）：20 个阶段边界均可重启恢复，迁移内部故障完整回滚，一次性备份保持可读 v1；本地模式 seed 7 个对象，My Tags 镜像模式只 seed 6 个对象；完全恢复后再次启动零重复 seed，临时文件完整清理。
...
- 2026-08-12 JSBox 真机数据库启动恢复临时库检查：通过（16 ms）。启动错误正确分级并脱敏，诊断只含 schema 元数据和文件状态，不含 Cookie、API Key、密码或业务行；原临时库与升级前临时备份保持不变，未执行自动恢复，临时文件已删除。
- 2026-08-12 JSBox 真机 v2 启动状态机临时库检查：通过（418 ms）。20 个阶段边界均可重启恢复，迁移内部故障完整回滚，一次性备份保持可读 v1；本地模式 seed 7 个对象，My Tags 镜像模式只 seed 6 个对象且不上传网站标签；完全恢复后再次启动不重复迁移或 seed，临时文件已删除。
...
- [x] 安装本次构建的开发版，进入“其他 → 云端同步”。
- [x] 点击“检查启动失败恢复与诊断脱敏”。
- [x] 确认结果显示启动错误已正确分级并脱敏，诊断不包含 Cookie、API Key、密码或业务行。
- [x] 确认原临时库与升级前临时备份保持不变、未执行自动恢复、临时文件已删除。
- [x] 回传“最近结果”。
...
- [x] 安装本次构建的开发版，进入“其他 → 云端同步”。
- [x] 点击“检查 v2 业务入口与稳定搜索 ID”。
- [x] 确认搜索历史和书签的稳定字符串 ID 能穿过 Config/UI facade，数字 v1 ID 会被 v2 明确拒绝。
- [x] 确认历史新增、本机删除、最近搜索词、书签新增/重排/tombstone 均正确。
- [x] 确认关闭重开后数据完整、临时文件已删除，并回传“最近结果”。
...
## v2 Repository 整库装配真机检查

- [x] 安装本次构建的开发版，进入“其他 → 云端同步”。
- [x] 点击“检查五类 v2 Repository 整库装配”。
- [x] 确认 7 个迁移对象可重入 seed，并覆盖 7 种同步实体。
- [x] 确认图库/阅读、搜索历史/书签、上传者和标签双模式关键路径均正确。
- [x] 确认 `sync_versions` 与 outbox 一致，关闭重开后数据完整、临时文件已删除，并回传“最近结果”。

预期结果：`v2 Repository 整库装配临时库检查通过（… ms）：五类 Adapter 在同一数据库、HLC writer 与 codec 下完成装配，7 个迁移对象可重入 seed，覆盖 7 种同步实体；ConfigManager/StatusManager 的图库、搜索历史与书签、上传者、标签双模式关键路径均正确，sync_versions 与 outbox 一致；关闭重开后数据完整，临时文件已删除。`
...
- [x] 安装本次构建的开发版，进入“其他 → 云端同步”。
- [x] 点击“检查 v2 启动状态机与中断恢复”。
- [x] 确认 20 个阶段边界均可重启恢复，迁移内部故障完整回滚，一次性备份保持可读 v1。
- [x] 确认本地模式 seed 7 个对象；My Tags 镜像模式只 seed 6 个对象且不上传网站标签。
- [x] 确认完全恢复后的再次启动不重复迁移或 seed、临时文件已删除，并回传“最近结果”。
...
1. 明确“尚未配置云同步”时 v2 Repository 的本机写入模式，以及首次开启同步时才创建加密 object key、outbox 和 seed 的边界；不得用诊断明文 codec 启动正式 App。
2. 设计只对正式数据库副本执行的全 App v2 启动与核心业务回归，覆盖启动、图库列表、阅读、搜索、标签、上传者、AI 与 WebDAV 本机行为。
3. 副本回归通过后，才把 v2 状态机接入正式 `DBManager`、提升 `CURRENT_USER_VERSION`，并再做一次真机升级验收。

## 整体路线位置与剩余里程碑

以“可供普通用户测试的端到端加密多设备同步 beta”为 v1 目标，不把 Phase 4 可选扩展计入发布阻断项：

| 阶段                               | 当前状态       | 剩余重点                                                                                         |
| ---------------------------------- | -------------- | ------------------------------------------------------------------------------------------------ |
| Phase 0：平台与风险验证            | 已完成         | 无发布阻断项                                                                                     |
| Phase 1：本地 DB v2 与 Repository  | 接近收尾       | 同步未配置模式、正式库副本全 App 回归、正式迁移接入                                              |
| Phase 2：同步内核与 Worker         | 部分基础已完成 | D1 objects/changes 规则、`/v1/sync`、push/pull、cursor/snapshot、配对/撤销/reset、多设备收敛测试 |
| Phase 3：生产加密、部署体验与 beta | 基础能力已验证 | 正式 E2EE codec 与密钥生命周期、恢复流程、错误/配额/UI、双真机 beta                              |
| Phase 4：AI/WebDAV 等扩展          | 可选，未开始   | 不阻塞第一版同步 beta                                                                            |

按较大的可验收里程碑估算，从现在到 v1 beta 还约有 **10 个主要里程碑**：Phase 1 收尾 3 个、Phase 2 约 5 个、Phase 3 约 2 个。这个数字是路线估算；一个主要里程碑仍可能像本阶段一样拆成多个小步。按工程量而非小步数量估算，当前约完成 v1 beta 的 **一半**，后半程主要是实际网络同步、多设备协议和生产安全，而不是继续增加本地表结构。
