...
- 书签内容：每个书签独立 LWW；
- 书签顺序：v1 使用固定宽度、带间隔的 base36 `position_key`（LexoRank 类方案），并用稳定书签 ID 打破相同位置；重排只为位置实际变化的书签逐项写版本。并发重排可能出现轻微顺序变化，但不得丢书签；未来需要任意相邻插入时可扩展 fractional 分配或在单设备空闲时显式重整；
