...
function defineASOHomeView() {
  ...
  const categoriesMatrix = categoriesView.defineCategoriesMatrix(
    layoutForCategories
  );
...
function defineASOFavoritesView(rawData) {
  ...
  const favoriteCategoriesMatrix = favoriteCategoriesView.defineFavoriteCategoriesMatrix(
    layoutForFavcats,
    rawData
  );
    ...
    views: [
      {
        ...
        events: {
          didSelect: function(sender, indexPath, data) {
            $("rootView")
              .get("listView")
              .get("textfield_search").text = data;
          }
        }
      },
      {
        ...
        events: {
          changed: function(sender) {
            ...
            const advancedSearchOptionsLocationView = sender.super.get(
              "advancedSearchOptionsLocationView"
            );
            ...
            if (sender.index === 0 || sender.index === 1) {
              ...
              advancedSearchOptionsLocationView.add(
                defineASOFavoritesView(glv.config["favcat_nums_titles"])
              );
