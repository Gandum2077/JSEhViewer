...
function handleQuery(query, downloads_order_method = "gid") {
  ...
  if (f_search) {
    ...
    if (query_tags.length && f_stags) {
      for (let i of query_tags) {
        ...
        condition_clauses.push(
          "EXISTS (SELECT tags.gid FROM tags WHERE downloads.gid=tags.gid AND tags.class=? AND (tags.tag=? OR tags.tag like ?))"
        );
    ...
    if (query_title.length && f_sname) {
      for (let i of query_title) {
        condition_clauses.push(
          "(downloads.japanese_title like ? OR downloads.english_title like ?)"
        );
        args.push("%" + i + "%", "%" + i + "%");
      }
    }
  ...
  if (f_cats) {
    ...
    condition_clauses.push(
      `downloads.category NOT IN ('${filtered_cat.join("', '")}')`
    );
  }
  if (f_srdd) {
  ...
  if (f_sp) {
    ...
    if (f_spt) {
      condition_clauses.push(
        `(${f_spf} <= downloads.length AND downloads.length <= ${f_spt})`
      );
    } else {
