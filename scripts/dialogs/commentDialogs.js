      ...
      },
      {
        ...
        events: {
          tapped: async function(sender) {
            ...
            const newInfos = await exhentaiParser.postNewComment(
              INFOS.url,
              text
            );
            if (newInfos) {
  ...
  views: [
    ...
    },
    {
      ...
      events: {
        tapped: async function(sender) {
          ...
          const result = await exhentaiParser.voteComment(
            INFOS.apikey,
            INFOS.apiuid,
            INFOS.gid,
            INFOS.token,
          ...
          if (result) {
            const this_comment = INFOS.comments.find(
              n => n.comment_id === comment_id
            );
            this_comment.my_vote = result.comment_vote
              ? result.comment_vote
              : null;
            this_comment.score =
              result.comment_score >= 0
    ...
    },
    {
      ...
      events: {
        tapped: async function(sender) {
          const comment_id = sender.super.get("label_title").info.comment_id;
          const result = await exhentaiParser.getEditComment(
            INFOS.apikey,
            INFOS.apiuid,
            INFOS.gid,
            INFOS.token,
          ...
          if (result) {
            ...
            const text = soup.rootElement.firstChild({ selector: "textarea" })
              .string;
            const newText = await textDialogs.textDialogs(
              $l10n("修改评论"),
              text
            );
            ...
            const newInfos = await exhentaiParser.postEditComment(
              INFOS.url,
              comment_id,
              newText
            );
            if (newInfos) {
              const comment_div = newInfos.comments.find(
                n => n.comment_id === comment_id
              ).comment_div;
