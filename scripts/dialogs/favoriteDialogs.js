...
function defineFavnoteView(favnote) {
  ...
  const footer = {
    ...
    props: {
      id: "footer",
      text:
        "此处最多只能写200字节（utf-8编码后的长度，英文1字节，汉字一般3字节）。中间换行无效",
      align: $align.left,
      font: $font(12),
      lines: 2,
      textColor: $color("black"),
    ...
    events: {
      didSelect: function(sender, indexPath, data) {
        ...
        sender.info = {
          ...
          favcat_title: favcat_titles.find(
            n => n.favcat === "favcat" + indexPath.item
          ).title
        };
      }
    }
...
async function favoriteDialogs(
  favcat_titles,
  favcat_selected,
  favnote,
  is_favorited
  ...
  return new Promise((resolve, reject) => {
    ...
    const confirmEvent = async function(sender) {
      ...
      const favcat_title = sender.super.super.get("favcatList").info
        .favcat_title;
    ...
    const titleBarView = baseViewsGenerator.defineTitleBarView(
      $l10n("收藏"),
      cancelEvent,
      confirmEvent
    );
    const favcatList = defineFavcatList(
      favcat_titles,
      favcat_selected,
      is_favorited
    );
...
module.exports = favoriteDialogs;
