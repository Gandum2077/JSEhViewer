...
var baseViewsForGalleryView = [
  {
    ...
    events: {
      tapped: async function(sender) {
        let alertTitle;
        if (
          parseInt(INFOS.length) ===
          $file.list(utility.joinPath(glv.imagePath, INFOS.filename)).length - 2
        ) {
          alertTitle = "是否保存为压缩包？";
        } else {
  ...
  },
  {
    ...
    props: {
      id: "button_info",
      image: $image("assets/icons/information_circled_64x64.png")
        .alwaysTemplate,
      tintColor: $color("#007aff"),
      bgcolor: $color("white"),
      imageEdgeInsets: $insets(12.5, 12.5, 12.5, 12.5)
  ...
  },
  {
    ...
    events: {
      tapped: async function(sender) {
        ...
        INFOS.pics.map(n => {
          old_pics_dict[n.key] = utility.joinPath(
            glv.imagePath,
            INFOS.filename,
            n.img_id + n.img_name.slice(n.img_name.lastIndexOf("."))
          );
          old_thumbnails_dict[n.key] = utility.joinPath(
            glv.imagePath,
            INFOS.filename,
            "thumbnails",
            n.img_id + ".jpg"
        ...
        INFOS.pics.map(n => {
          new_pics_dict[n.key] = utility.joinPath(
            glv.imagePath,
            INFOS.filename,
            n.img_id + n.img_name.slice(n.img_name.lastIndexOf("."))
          );
          new_thumbnails_dict[n.key] = utility.joinPath(
            glv.imagePath,
            INFOS.filename,
            "thumbnails",
            n.img_id + ".jpg"
        ...
        for (let key in old_pics_dict) {
          if (
            $file.exists(old_pics_dict[key]) &&
            key in new_pics_dict &&
            !$file.exists(new_pics_dict[key])
          ) {
            $file.move({
          ...
          if (
            $file.exists(old_thumbnails_dict[key]) &&
            key in new_thumbnails_dict &&
            !$file.exists(new_thumbnails_dict[key])
          ) {
            $file.move({
        ...
        $ui.window
          .get("galleryView")
          .get("button_try_import_old_version").hidden = true;
      }
    }
  },
  {
    ...
    events: {
      tapped: function(sender) {
        ...
        const old_infosFile = utility.joinPath(
          glv.imagePath,
          old_filename,
          "manga_infos.json"
        );
  ...
  const baseViewsForGalleryInfoView = [
    {
      ...
      props: {
        ...
        src: utility.joinPath(
          glv.imagePath,
          INFOS.filename,
          "thumbnails",
          INFOS.pics[0].img_id + ".jpg"
        ),
        contentMode: 1,
    ...
    },
    {
      ...
      props: {
        ...
        bgcolor: $color(
          utility.getNameAndColor(INFOS["category"].toLowerCase())["color"]
        )
      },
      layout: function(make, view) {
    ...
    },
    {
      ...
      events: {
        ready: async function(sender) {
          ...
          while (
            sender.super &&
            $file.list(path).length - 2 < parseInt(INFOS.pics.length)
          ) {
            sender.get("inner").frame = $rect(
              0,
              0,
              (sender.frame.width * ($file.list(path).length - 2)) /
                parseInt(INFOS.pics.length),
              36
            );
            await $wait(1);
          }
    ...
    },
    {
      ...
      events: {
        layoutSubviews: sender => {
          ...
          inner.bgcolor = $color(
            INFOS["is_personal_rating"] ? "#5eacff" : "#ffd217"
          );
        }
      }
    },
    {
      ...
      events: {
        tapped: async function(sender) {
          const rating = await ratingAlert({ rating: INFOS.display_rating });
          utility.startLoading();
          const success = await exhentaiParser.rateGallery(
            rating,
            INFOS.apikey,
            INFOS.apiuid,
            INFOS.gid,
    ...
    },
    {
      ...
      props: {
        id: "label_review",
        text:
          INFOS["rating"] + " on " + INFOS["number of reviews"] + " reviews",
        font: $font(12),
        align: $align.center,
        textColor: $color("black"),
        bgcolor: $color("white")
    ...
    },
    {
      ...
      props: {
        ...
        bgcolor: INFOS["favcat"]
          ? $color(utility.getColorFromFavcat(INFOS["favcat"]))
          : $color("white")
      },
      layout: function(make, view) {
        make.height.equalTo(36);
      ...
      events: {
        tapped: async function(sender) {
          utility.startLoading();
          const favInfos = await exhentaiParser.getFavcatAndFavnote(
            INFOS["url"]
          );
          utility.stopLoading();
          const result = await favoriteDialogs(
            favInfos.favcat_titles,
            favInfos.favcat_selected,
            favInfos.favnote,
            favInfos.is_favorited
          );
          const old_is_favorited = favInfos.is_favorited;
          utility.startLoading();
          const success = await exhentaiParser.addFav(
            INFOS["url"],
            result.favcat,
            result.favnote,
            old_is_favorited
          ...
          if (success) {
            ...
            sender.textColor = INFOS["favcat"]
              ? $color("white")
              : $color("black");
            sender.bgcolor = INFOS["favcat"]
              ? $color(utility.getColorFromFavcat(INFOS["favcat"]))
              : $color("white");
            const path = utility.joinPath(glv.imagePath, INFOS.filename);
            exhentaiParser.saveMangaInfos(INFOS, path);
...
function defineFullTagTableView(width, translated = true) {
  ...
  const views = [
    ...
    },
    {
      ...
      events: {
        tapped: function(sender) {
          ...
          const newTagTableView = tagTableViewGenerator.defineTagTableView(
            width - 51,
            bilingualTaglist,
            translated
          );
    ...
    },
    {
      ...
      events: {
        tapped: function(sender) {
          const label_uploader = $ui.window
            .get("galleryView")
            .get("galleryInfoView")
            .get("label_uploader");
          const selectedTags = [];
...
function defineHeaderView() {
  const width =
    Math.min($device.info.screen.width, $device.info.screen.height) - 57 - 16;
  ...
  const views = [fullTagTableView, commentsView];
  const headerView = {
    type: "view",
    props: {
...
async function refresh(newUrl, getNewInfos = true) {
  ...
  if ($file.list(path).length - 2 > 0 && INFOS.newer_versions) {
    ...
    button_update.info = {
      url: INFOS.newer_versions[INFOS.newer_versions.length - 1][0]
    };
  } else {
    button_update.hidden = true;
  }
  // 导入旧版按钮
  const button_try_import_old_version = $ui.window
    .get("galleryView")
    .get("button_try_import_old_version");
  if ($file.list(path).length - 2 === 0 && INFOS.parent_url) {
    ...
    if (result.length) {
      ...
      const clause = `SELECT DISTINCT gid||'_'||token
                      ...
                      LIMIT 1
                      `;
      const args = [INFOS["uploader"], INFOS["english_title"], INFOS["gid"]];
      const result = database.search(clause, args);
      if (result.length) {
        filename = result[0]["gid||'_'||token"];
      }
...
function checkIsDownloadUnfinished() {
  ...
  const unfinished =
    counts.EXECUTING + counts.QUEUED + counts.RUNNING + counts.RECEIVED;
  if (unfinished) {
    return true;
  } else {
    return false;
  ...
  const rootView = {
    ...
    events: {
      ...
      disappeared: function() {
        ...
        // 存入数据库
        if (
          $file.list(utility.joinPath(glv.imagePath, INFOS.filename)).length -
            2 >
          0
        ) {
