...
const baseViewsForListView = [
  {
    ...
    events: {
      tapped: function(sender) {
        $("rootView")
          .get("listView")
          .get("sidebarView").hidden = !$("rootView")
          .get("listView")
          .get("sidebarView").hidden;
      }
    }
  },
  {
    ...
    events: {
      tapped: async function(sender) {
        ...
        if (asv) {
          ...
          const query = {};
          const searchPhrase = $("rootView")
            .get("listView")
            .get("textfield_search").text;
          if (searchPhrase) {
            query["f_search"] = searchPhrase;
          }
          if (option === 0 || option === 1) {
            const ASOHomeView = asv
              .get("advancedSearchOptionsLocationView")
              .get("ASOHomeView");
            let f_cats = 0;
            ...
            const advsearch = ASOHomeView.get("advancedSearchOptionSwitch").info
              .advsearch;
            if (advsearch) {
            ...
            const ASOFavoritesView = asv
              .get("advancedSearchOptionsLocationView")
              .get("ASOFavoritesView");
            const favcatItem = ASOFavoritesView.get(
              "favoriteCategoriesMatrix"
            ).data.find(n => {
            ...
            const ASODownloadsView = asv
              .get("advancedSearchOptionsLocationView")
              .get("ASODownloadsView");
            let f_cats = 0;
            ...
            const advsearch = ASODownloadsView.get("advancedSearchOptionSwitch")
              .info.advsearch;
            if (advsearch) {
              query["advsearch"] = advsearch;
              Object.assign(
                query,
                ASODownloadsView.get("optionsDownloads").info
              );
            }
          }
          if (!query["f_sr"]) {
  ...
  },
  {
    ...
    events: {
      tapped: async function(sender) {
        ...
        const url = await inputAlert({ title: "直接打开", text });
  ...
  },
  {
    ...
    events: {
      tapped: async function(sender) {
        const current_page_str = $("rootView")
          .get("listView")
          .get("button_jump_page")
          .get("label_current_page").text;
        const total_pages_str = $("rootView")
          .get("listView")
          .get("button_jump_page")
        ...
        if (/\d+-\d+/.test(current_page_str)) {
          nowPage =
            parseInt(
              current_page_str.slice(current_page_str.indexOf("-") + 1)
            ) - 1;
        } else {
...
function getData(items) {
  ...
  for (let item of items) {
    const taglist =
      typeof item["taglist"] === "string"
        ? JSON.parse(item["taglist"])
        : item["taglist"];
    const itemData = {
      thumbnail_imageview: {
      ...
      label_posted: {
        text: item["posted"],
        borderColor: item["favcat"]
          ? $color(utility.getColorFromFavcat(item["favcat"]))
          : $color("white")
      },
      delete_line_view: {
      ...
      actions: [
        {
          ...
          handler: function(sender, indexPath) {
            ...
            $file.delete(
              utility.joinPath(glv.imagePath, `${deleted.gid}_${deleted.token}`)
            );
...
async function refresh(newUrl) {
  ...
  $("rootView")
    .get("listView")
    .get("realListView").data = getData(infos["items"]);
  $("rootView")
    .get("listView")
    .get("realListView").header.text = infos["search_result"];
  $("rootView")
    .get("listView")
    .get("realListView")
    .scrollTo({
  ...
  $("rootView")
    .get("listView")
    .get("button_sidebar").image = getSideBarButtonImage(urlCategory);
  $("rootView")
    .get("listView")
    .get("button_jump_page")
    .get("label_current_page").text = infos["current_page_str"];
  $("rootView")
    .get("listView")
    .get("button_jump_page")
    .get("label_total_page").text = infos["total_pages_str"];
  $("rootView")
    .get("listView")
    .get("textfield_search").text = utility.parseUrl(URL).query.f_search || "";
...
async function presentSettings() {
      ...
      fields: [
        {
          ...
          value: async () => {
            const login = await loginAlert({ title: "登录" });
            $file.write({
        ...
        },
        {
          ...
          value: async () => {
            ...
            if (alert.index) {
              ...
              const success = await exhentaiParser.login(
                login.username,
                login.password
              );
              if (!success) {
          ...
          value: async () => {
            ...
            if (alert.index) {
              ...
              for (let filename of $file.list(glv.imagePath)) {
                if (
                  $file.list(utility.joinPath(glv.imagePath, filename)).length -
                    2 >
                  0
                ) {
            ...
            if (alert.index) {
              for (let folder of $file.list(glv.imagePath)) {
                const infos = JSON.parse(
                  $file.read(
                    utility.joinPath(glv.imagePath, folder, "manga_infos.json")
                  ).string
                );
                if (!infos["favcat"]) {
  ...
  const favorites_order_method =
    favorites_order_method_items[result.favorites_order_method];
  ...
  glv.config.downloads_order_method =
    downloads_order_method_items[result.downloads_order_method];
