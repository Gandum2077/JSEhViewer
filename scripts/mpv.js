            ...
            fields: [
              ...
              },
              {
                ...
                value: async () => {
                  ...
                  const newInfos = await exhentaiParser.getMpvInfosFromUrl(
                    INFOS.url
                  );
                  utility.stopLoading();
    ...
    events: {
      ready: async function(sender) {
        ...
        while (sender.super && $file.list(path).length - 2 < length) {
          ...
          sender.get("inner").info = {
            progress: ($file.list(path).length - 2) / length
          };
          sender
            .get("inner")
            .runtimeValue()
        ...
        events: {
          ready: async function(sender) {
            ...
            sender.frame = $rect(
              0,
              0,
              sender.super.frame.width,
              (sender.super.frame.height * value) / max
            );
          }
        }
      }
...
function defineSlider() {
  ...
  const slider = defineCustomSlider(
    layout,
    {
      id: "slider1",
...
function changeValueOfSlider(value) {
  ...
  inner.frame = $rect(
    ...
    0,
    wrapper.frame.width,
    (wrapper.frame.height * value) / slider.info.max
  );
}
...
function getPicPath() {
  ...
  const picPath = utility.joinPath(
    path,
...
function refreshMpv(ignoreSlider = false) {
  ...
  if ($file.exists(picPath)) {
    ...
    mpv
      .get("scroll")
      .get("contentView")
      .get("image").src = picPath;
    mpv.get("spinner").loading = false;
  } else {
    mpv
      .get("scroll")
      .get("contentView")
      .get("image").src = undefined;
        ...
        if (sender.super.zoomScale === 1) {
          const newPage =
            location.y <= sender.frame.height / 2
              ? Math.max(PAGE - 1, 1)
              : Math.min(PAGE + 1, INFOS.pics.length);
          if (PAGE !== newPage) {
            PAGE = newPage;
            refreshMpv();
          }
  ...
  const mpv = {
    ...
    events: {
      ready: async function(sender) {
        ...
        await $wait(0.05);
        mpv.get("scroll").get("contentView").frame = $rect(
          0,
          0,
