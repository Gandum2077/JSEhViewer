...
function adjustSize(size, maxWidth) {
  const width = Math.min(
    size.width < 26 ? 30 : Math.ceil(size.width + 4),
    maxWidth
  );
...
function defineTagTableView(
  ...
  for (let idx in tagsViewsArray) {
    ...
    tagsView.props.frame = $rect(
      90 + 5,
      cumulatedHeight + verticalMargin,
      tagsView.props.size.width,
      tagsView.props.size.height
    );
    ...
    const tagTypeLabel = {
      ...
      props: {
        ...
        frame: $rect(
          ...
          cumulatedHeight,
          90,
          tagsView.props.frame.height + verticalMargin * 2
        )
      }
    };
    const lowlevelView = {
      ...
      props: {
        ...
        info: {
          ...
          height: tagsView.props.frame.height + verticalMargin * 2
        }
      },
