...
const sections = [
  {
    ...
    footer: $device.isIpad
      ? $l10n("满足全部诉求后，点击完成输入账号密码")
      : $l10n("很遗憾，您的设备不是iPad"),
    fields: [
      {
        type: "info",
        ...
        icon: $image(
          $device.isIpad
            ? "assets/icons/ipad_64x64.png"
            : "assets/icons/close_64x64.png"
        )
      },
      {
        type: "action",
...
async function init() {
  ...
  try {
    await formDialogs({ sections, title: $l10n("初始设置") });
  } catch (err) {
    $app.close();
  }
  let login;
