...
async function init(url?: string) {
  ...
  if (!configManager.cookie) {
    // 重新登录时取消旧账号的待恢复任务，保留本机下载标记。
    dbManager.update("UPDATE archive_download_state_v2 SET finished = 1 WHERE finished = 0");
    await login();
    appLog("login done");
  ...
  // 保留未完成任务，恢复失败或中途退出时下次启动仍可重试。
  const lastDownloads = getPendingDownloads();
  ...
  if (configManager.resumeIncompleteDownloadsOnStart) {
    ...
    lastDownloads.forEach((n) => {
      ...
      if ($file.list(image_path)?.length === n.length) {
        dbManager.update("UPDATE archive_download_state_v2 SET finished = 1 WHERE id = ?", [String(n.gid)]);
        return;
      }
      const item = statusManager.getArchiveItem(n.gid);
